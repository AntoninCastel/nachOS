...
\begin{itemize}
	...
	\item Une map qui index les pids de ses fils à leurs sémaphores
\end{itemize}
Les conséquences de cette organisation sont que les pids sont réservés aux processus pères, ainsi, un processus ne peut pas contrôler les processus fils de ses fils, car les pids auquel il a accès lui sont locaux.\\
Les pids commencent à zéro pour tous les processus et il n'y a pas de nombre maximum de processus tant que la mémoire physique permet de les charger.\\
...
Lorsqu'un processus se termine, il attend la fin de tous ses fils. Il le fait par le biais des sémaphores auquel il a accès. Celà implique l'arrêt de ses petits-fils, car le fils du processus principal ne peut se terminer que si ses propres fils sont terminés.
...
Il est à noter que les noms ".", "..", "/" ainsi que la chaine de caractères vide ne sont pas des noms de fichier (ou répertoires) corrects.
...
Comme la table est globale, il se peut qu'il y ai des conflits entre deux processus qui voudraient accéder au même fichier, si l'un d'entre eux ferme le fichier avant que le second écrive dedans.\\
Dans ce cas, le second programme n'aura plus accès au fichier.
