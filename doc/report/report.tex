...
\myparagraph{Sem\_Init}
	...
	\textsc{Synopsis}\\
		\texttt{semaphore_t Sem\_Init(int n)}\\
...
\myparagraph{Sem\_P}\\
\textsc{Synopsis}\\	
	\texttt{int Sem\_P(semaphore_t s)}\\
...
\texttt{int Sem\_V(semaphore_t s)}\\
...
\textsc{Synopsis}\\	
	\texttt{int Sem\_GetValue(semaphore_t s)}\\
...
\textsc{Synopsis}\\	
	\texttt{int Sem\_Destroy(semaphore_t s)}\\
